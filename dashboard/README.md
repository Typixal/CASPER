...
│       └── components/     Panels + hand-built SVG charts (no charting lib)
...
the committed `static/dist/`. No CDN, no charting library: React and Tailwind
are compiled into that bundle at build time (see below), and every chart
(latency, replica timeline, source split) is hand-built inline SVG.
